...
namespace yuxuetian
{
    public class ShaderReferenceRenderState : EditorWindow
    {
        ...
        {
            _reference.DrawTitle("Cull" , "https://docs.unity3d.com/cn/2023.2/Manual/SL-Cull.html");
        }
        ...
        {
            _reference.DrawTitle("Stencil Test(模板测试)");
        }
        public void DrawContentStencilTest(bool isFold)
        {
            if (isFold)
            {
                _reference.DrawContent("Stencil", "模板缓冲区(StencilBuffer)可以为屏幕上的每个像素点保存一个无符号整数值,这个值的具体意义视程序的具体应用而定.在渲染的过程中,可以用这个值与一个预先设定的参考值相比较,根据比较的结果来决定是否更新相应的像素点的颜色值.这个比较的过程被称为模板测试." +
                ...
                _reference.DrawContent("模版缓冲区的更新",
        ...
        {
            _reference.DrawTitle("Depth Test(深度测试)");
        }
        public void DrawContentDepthTest(bool isFold)
        {
            if (isFold)
            {
                _reference.DrawContent("ZTest (Less | Greater | LEqual | GEqual | Equal | NotEqual | Never | Always)", "深度测试，拿当前像素的深度值与深度缓冲中的深度值进行比较，默认值为LEqual。" +
                                         ...
                                         "\nGequal：大于等于。" +
                                         "\nAlways：永远通过。");
                _reference.DrawContent("ZTest[unity_GUIZTestMode]", "unity_GUIZTestMode用于UI材质中，此值默认为LEqual,仅当UI中Canvas模式为Overlay时，值为Always.");
                _reference.DrawContent("ZWrite On | Off", "深度写入，默认值为On。\nOn：向深度缓冲中写入深度值。\nOff：关闭深度写入。");
                _reference.DrawContent("ZClip True | False", "设置GPU的深度剪辑模式以此来决定如何处理近平面和远平面之外的片元深度.\nFalse表示将GPU的深度剪辑模式设置为Clmap,这对于模板阴影渲染很有用,这意味着当几何体超出远平面时不需要特殊处理，从而减少渲染操作。但是，它可能会导致不正确的Z排序。");
                _reference.DrawContent("Offset Factor, Units", "深度偏移，offset = (m * factor) + (r * units)，默认值为0,0" +
        ...
        {
            _reference.DrawTitle("ColorMask(颜色遮罩)");
        }
        ...
        {
            _reference.DrawTitle("Blend(混合)");   
        }
        public void DrawContentBlend(bool isFold)
        {
            if (isFold)
            {
                _reference.DrawContent("说明", "源颜色*SrcFactor + 目标颜色*DstFactor" + "\n颜色混合，源颜色与目标颜色以给定的公式进行混合出最终的新颜色。" +
                                         "\n源颜色：当前Shader计算出的颜色。" +
                ...
                _reference.DrawContent("Blend SrcFactor DstFactor", "SrcFactor为源颜色因子，DstFactor为目标颜色因子，将两者按Op中指定的操作进行混合。");
                _reference.DrawContent("Blend SrcFactor DstFactor, SrcFactorA DstFactorA", "对RGB和A通道分别做混合操作.\n对A的混合操作结果可以在FrameDebugger中的渲染目标RT中点选A通道来查看.");
                _reference.DrawContent("BlendOp Op", "混合时的操作运算符，默认值为Add（加法操作）。");
                _reference.DrawContent("BlendOp OpColor, OpAlpha", "对RGB和A通道分别指定混合运算符。");
                _reference.DrawContent("Blend factors", "混合因子" +
                                         "\nOne：源或目标的完整值" +
                                         "\nZero：0" +
                                         "\nSrcColor：源的颜色值" +
                                         "\nSrcAlpha：源的Alpha值" +
                ...
                _reference.DrawContent("Blend Types", "常用的几种混合类型" +
                ...
                _reference.DrawContent("Blend operations", "混合操作的具体运算符" +
                                         "\nAdd：源+目标" +
                                         "\nSub：源-目标" +
                                         "\nRevSub：目标-源" +
                                         "\nMin：源与目标中最小值" +
        ...
        {
            _reference.DrawContent("AlphaToMask On | Off", "是否启用GPU上的alpha-to-coverage模式(当开启MSAA时,减少AlphaTest产生的过度锯齿感).");
            _reference.DrawContent("Conservative True | False", "是否启用保守光栅化(指GPU对被三角形部分覆盖的像素进行光栅化,无论覆盖范围如何,这会导致更多片元着色器的调用).");
        }
    }
}

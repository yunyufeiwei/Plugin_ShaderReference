...
namespace Editor.ShaderDocument
{
    public class ShaderReferenceBuildInVariables : EditorWindow
    {
        ...
        {
            _reference.DrawTitle("Vert");
        }

        public void DrawContentVert()
        {
            _reference.DrawContent("UNITY_INITIALIZE_OUTPUT(type,name)", "由于HLSL编缉器不接受没有初始化的数据，所以为了支持所有平台，从而需要使用此方法进行初始化.\n" +
                                                                         "Varying o = (Varying)0");
            _reference.DrawContent("o.uv = TRANSFORM_TEX(i.uv,_MainTex)","对UV进行Tiling与Offset变换,也可以将其拆开。\n"+
                                                                         "o.uv = v.texcoord.xy * _BaseMap_ST.xy + _BaseMap_ST.zw");
            _reference.DrawContent("float3 UnityWorldSpaceLightDir( float3 worldPos )", "返回顶点到灯光的向量");
        }

        public void DrawTitleBuildInVariabledCameraAndScreen()
        {
            _reference.DrawTitle("CameraAndScreen");
        }
        ...
        public void DrawContentBuildInVariabledCameraAndScreen(bool isFold)
        {
            if (isFold)
            {
                _reference.DrawContent("Camera相关参数", "相机在世界空间下的位置坐标(xyz):_WorldSpaceCameraPos\n" +
                ...
                _reference.DrawContent("_ZBufferParams", "传统Z方向:\n" +
                                                         "x=1-far/near\n" +
                                                         "y=far/near\n" +
                                                         "z=x/far\n" +
                                                         "w=y/far\n\n" +
                                                         ...
                                                         "z=x/far\n" +
                                                         "w=1/far");
                _reference.DrawContent("深度:_CameraDepthTexture", "在PipelineAsset中勾选DepthTexture.\n" +
                ...
                _reference.DrawContent("线性深度转换","从深度图中得到顶点的线性深度值(相机位置=0，相机远裁剪面=1)\n" +
                                                "Linear01Depth(depthMap, _ZBufferParams);\n" +
                                                "从深度图中得到顶点的线性深度值(不是0-1的范围)\n" +
                                                "LinearEyeDepth(depthMap, _ZBufferParams);");
                _reference.DrawContent("抓屏:_CameraOpaqueTexture", "在PipelineAsset中勾选OpaqueTexture,同时这个抓屏只能在半透明序列下正确执行.\n" +
                ...
                _reference.DrawContent("_ScreenParams", "屏幕的相关参数，单位为像素。\nx表示屏幕的宽度\ny表示屏幕的高度\nz表示1+1/屏幕宽度\nw表示1+1/屏幕高度");
                _reference.DrawContent("_ScaledScreenParams", "同上，但是有考虑到RenderScale的影响.");
            }
        ...
        public void DrawContentBuildInVariablesTime(bool isFold)
        {
            if (isFold)
            {
                _reference.DrawContent("_Time", "时间，主要用于在Shader做动画,类型：float4\nx = t/20\ny = t\nz = t*2\nw = t*3");
                _reference.DrawContent("_SinTime", "t是时间的正弦值，返回值(-1~1): \nx = t/8\ny = t/4\nz = t/2\nw = t");
                _reference.DrawContent("_CosTime", "t是时间的余弦值，返回值(-1~1):\nx = t/8\ny = t/4\nz = t/2\nw = t");
                _reference.DrawContent("unity_DeltaTime", "dt是时间增量,smoothDt是平滑时间\nx = dt\ny = 1/dt\nz = smoothDt\nz = 1/smoothDt");
        ...
        {
            _reference.DrawTitle("GPU Instancing");
        }
        ...
        public void DrawContentBuildInVariablesGPUInstancing(bool isFold)
        {
            if (isFold)
            {
                _reference.DrawContent("实现步骤", "用于对多个对象(网格一样，材质一样，但是材质属性不一样)合批,单个合批最大上限为511个对象.\n" +
                ...
                _reference.DrawContent("Instancing选项", "对GPU Instancing进行一些设置.\n" +

...
namespace Editor.ShaderDocument
{
    public class ShaderReferencePipeline : EditorWindow
    {
        ...
        public void DrawContentApplicationStage(bool isFold)
        {
            if (isFold)
            {
                _reference.DrawContent("1.Application Stage", "  此阶段一般由CPU将需要在屏幕上绘制的几何体、摄像机位置、光照纹理等输出到管线的几何阶段.");
            }
        }
        ...
        public void DrawContentResterizerStage(bool isFold)
        {
            if (isFold)
            {
                ...
                _reference.DrawContent("4.混合（Merging）", "主要任务是合成当前储存于缓冲器中的由之前的像素着色阶段产生的片段颜色。此阶段还负责可见性问题（深度测试、模版测试等）的处理.");

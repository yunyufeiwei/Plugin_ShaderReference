...
namespace Editor.ShaderDocument
{
    public class ShaderReferenceMath : EditorWindow
    {
        ...
        {
            _reference.DrawTitle("Texture Sampler(普通纹理采样)");
        }
        ...
        public void DrawContentMathTextureSampler(bool isFold)
        {
            if (isFold)
            {
                _reference.DrawContent("TEXTURE2D (textureName);", "二维纹理的定义(纹理与采样器分离定义),此功能在OpenGL ES2.0上不支持，会使用原来sampler2D的形式.\n" +
                                         "textureName:Properties中声明的2D纹理.");
                _reference.DrawContent("TEXTURECUBE (textureName);", "立方体纹理的定义(纹理与采样器分离定义),此功能在OpenGL ES2.0上不支持，会使用原来samplerCube的形式.\n" +
                                         "textureName:Properties中声明的立方体纹理.");
                _reference.DrawContent("SAMPLER(samplerName);", "采样器的定义(纹理与采样器分离定义),采样器是指纹理的过滤模式与重复模式,此功能在OpenGL ES2.0上不支持，相当于没写.\n" +
                ...
                _reference.DrawContent("SAMPLE_TEXTURE2D_LOD(textureName, samplerName, coord,lod);", "进行二维纹理采样操作\n" +
                                         ...
                                         "coord:采样用的UV\n" +
                                         "lod:mipmap级别");
                _reference.DrawContent("SAMPLE_TEXTURE2D_BIAS(textureName, samplerName, coord,bias);", "对mipmap进行偏移后再采样纹理\n" +
                                         "textureName:Properties中声明的2D纹理名称\n" +
                                         "samplerName:此纹理所使用的采样器设置\n" +
                                         "coord:采样用的UV\n" +
